...
process dsmix {
  ...
  script:
  """
  if [[ -f "old/${oldDS[0].baseName}.yml" ]]; then
    ...
    tips convert ${space_sep(newDS)} -f asetraj -o tmp.traj -of asetraj
    tips convert tmp.traj -f asetraj -o new-ds -of asetraj $newFlag
    tips convert new-ds.traj -f asetraj -o mix-ds -of pinn --shuffle $params.filters
    rm {new-ds,tmp}.*
  fi
  """
}
...
process check {
  ...
  enum = e_pred.size # for the calcuation of total RMSE without ase trajectory
  fnum = f_pred.size # for the calcuation of total RMSE without ase trajectory
  esae = np.sum(np.abs(e_pred-e_label)) # for the calcuation of total MAE without ase trajectory
  fsae = np.sum(np.abs(f_pred-f_label)) # for the calcuation of total MAE without ase trajectory
  esse = np.sum((e_pred-e_label)**2) # for the calcuation of total RMSE without ase trajectory
  fsse = np.sum((f_pred-f_label)**2) # for the calcuation of total RMSE without ase trajectory
  ...
  if converged:
      msg = f'Converged: will restart from latest frame; ' \
            f'energy: max={emax:.6f} rmse={ermse:.6f} enum={enum:.1f} esae={esae:.6f} esse={esse:.8f}; '\
            f'force: max={fmax:.6f} rmse={frmse:.6f} fnum={fnum:.1f} fsae={fsae:.6f} fsse={fsse:.8f}.'
      new_geo = logs[np.argmax(idx)]
  else:
      msg = f'energy: {ecnt}/{len(idx)} failed to meet emaxtol, max={emax:.6f} rmse={ermse:.6f} enum={enum:.1f} esae={esae:.6f} esse={esse:.8f}; '\
            f'force: {fcnt}/{len(idx)} failed to meet fmaxtol, max={fmax:.6f} rmse={frmse:.6f} fnum={fnum:.1f} fsae={fsae:.6f} fsse={fsse:.8f}.'
