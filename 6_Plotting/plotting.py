...
from setting import proj_params,plot_params
...
plt.rc('font', family='sans-serif', size=plot_params["default_font_size"])
...
def drawLearningCurve(nSeed):
    ...
    pinnacle_output_dir = proj_params["pinet2_ft_dir"] / f"output-cp2k-from-model-seed{nSeed}"
    ...
    # do the plot
    f, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(5, 1, figsize=[plot_params["double_col_width"], 7], 
                                    sharex=True, 
    ...
    strOutFile = plot_params["figure_dir"] / f"learning_curve_seed{nSeed}.jpg"
    plt.savefig(fname=strOutFile, dpi=plot_params["resolution_dpi"])
    plt.close()
...
def drawNetForceAndAtomicDress(nSeed):
    ...
    # do the plot
    f, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=[plot_params["double_col_width"], 5], 
    ...
    strOutFile = plot_params["figure_dir"] / f"net_force_e_dress_seed{nSeed}.jpg"
    plt.savefig(fname=strOutFile, dpi=plot_params["resolution_dpi"])
    plt.close()
